...
|---|---|
|Compiler API|`compile(sexpr, { functions, onDiagnostic })` returns a reusable zero-argument function. A root value invoke returns its value; a root block or none-valued invoke returns JavaScript `undefined`, which is not a Script IR value.|
...
|External functions|Names resolve to fixed numeric slots while compiling. Results must be logical values; JavaScript `undefined` is rejected rather than becoming a second null-like value. Standard names cannot be overridden.|
