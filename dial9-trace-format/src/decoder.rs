...
//!
//! [`Decoder`] reads a complete byte slice and yields frames as
//! [`DecodedFrame`] (owned) or [`DecodedFrameRef`] (zero-copy).
//! [`StreamingDecoder`] accepts arbitrary chunks and yields events while
//! retaining only an incomplete frame between calls.

use crate::codec::{
    self, Frame, FrameRef, HEADER_SIZE, PoolEntry, PoolEntryRef, SchemaInfo, StackPoolEntry,
    StackPoolEntryRef, ViewerExtension, ViewerExtensionRef, WireTypeId,
};
...
impl<'a> Decoder<'a> {
    ...
    pub fn next_frame(&mut self) -> Result<Option<DecodedFrame>, DecodeError> {
        ...
        match frame {
            ...
            Frame::ViewerExtension(extension) => Ok(Some(DecodedFrame::ViewerExtension(extension))),
        }
    }
    ...
    pub fn next_frame_ref(&mut self) -> Result<Option<DecodedFrameRef<'a>>, DecodeError> {
        ...
        match frame {
            ...
            FrameRef::ViewerExtension(extension) => {
                Ok(Some(DecodedFrameRef::ViewerExtension(extension)))
            }
        }
    }
...
impl StreamingDecoder {
    ...
    ///
    /// Schemas, pools, viewer extensions, and reset headers are consumed
...
mod tests {
    ...
    #[test]
    fn viewer_extensions_are_exposed_by_frame_decoders_and_skipped_by_event_decoders() {
        let mut enc = Encoder::new();
        enc.write_viewer_extension("cpu", b"\0asm").unwrap();
        let schema = enc
            .register_schema(
                "Ev",
                vec![FieldDef {
        ...
        let mut owned = Decoder::new(&data).unwrap();
        assert!(matches!(
            owned.next_frame().unwrap(),
            Some(DecodedFrame::ViewerExtension(ref extension))
                if extension.name == "cpu" && extension.wasm == b"\0asm"
        ));

        let mut borrowed = Decoder::new(&data).unwrap();
        assert!(matches!(
            borrowed.next_frame_ref().unwrap(),
            Some(DecodedFrameRef::ViewerExtension(extension))
                if extension.name == "cpu" && extension.wasm == b"\0asm"
        ));

        let mut event_iter = Decoder::new(&data).unwrap();
        assert_eq!(
            event_iter
                .events()
        ...
        let mut callback_decoder = Decoder::new(&data).unwrap();
        let mut event_count = 0;
        callback_decoder
            .for_each_event(|_| event_count += 1)
            .unwrap();
    ...
    fn streaming_test_trace() -> Vec<u8> {
        ...
        first.write_viewer_extension("first", b"\0asm").unwrap();
        ...
        // A complete second trace exercises a reset header and fresh schema
        // state in the same byte stream.
    ...
    fn streaming_decoder_rejects_every_truncated_event_tail() {
        ...
        let event_start = prefix_decoder.pos;
        ...
        for cut in event_start + 1..data.len() {
            ...
            assert_eq!(decoder.tail.len(), cut - event_start);
