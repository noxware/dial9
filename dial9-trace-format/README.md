...
The `#[traceevent(kind = "...")]` attribute describes how viewers should graph a field: `"gauge"` plots absolute samples, `"counter"` plots non-negative deltas and treats decreases as resets, and `"up_down_counter"` plots signed deltas. Anything else is a compile error.
